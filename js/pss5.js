...
const JUMLAH_PERTANYAAN = 5;
const BATAS_SKOR_TINGGI = 15; // skor >= ini dianggap stres TINGGI

const form = document.getElementById('formPss5');
...
form.addEventListener('submit', function (event) {
  ...
  let skor = 0;
  const jawaban = {}; // disimpan mentah, bukan cuma skor, supaya bisa dipulihkan
  for (let i = 1; i <= JUMLAH_PERTANYAAN; i++) {
  ...
  const status = skor >= BATAS_SKOR_TINGGI ? 'TINGGI' : 'RENDAH';
